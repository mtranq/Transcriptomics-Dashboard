...
anno_reactive <- reactive({
  ...
  colData(vsd)[,c("tissue_type","ajcc_pathologic_stage","primary_diagnosis","race", "ethnicity")] %>% as.data.frame()
})
```
...
renderPlot({
  ...
  if (is.null(sampleDistMatrix) || is.null(anno)) {
    return(
      ggplot() +
      geom_text(aes(x = 0.5, y = 0.6, label = "⚠️ Heatmap Unavailable"),
                color = "red", size = 6) +
...
renderPlotly({
  ...
  if (is.null(pca_data)) {
      plotly::plotly_empty() %>%
        layout(
          title = list(text = "📊 Not Enough Samples Selected<br><sub>Please select at least 3 patients,\nthen click 'Apply Samples' to view the PCA plot.</sub>", 
          font = list(size = 16)),
...
renderPlotly({
  sep_pca_data <- separated_pca_reactive()
    ...
    if (is.null(sep_pca_data)) {
      plotly::plotly_empty() %>%
        layout(
          title = list(text = "📊 Not ENough Samples Selected<br><sub>Please select at least 3 samples per condition,\nthen click 'Apply Samples' to view the PCA plot.</sub>", 
          font = list(size = 16)),
          xaxis = list(visible = FALSE),
          yaxis = list(visible = FALSE)
        )
...
res_reactive <- reactive({
  ...
  res <- results(dds, name = "vital_status_Dead_vs_Alive")
  return(res)
})
```
...
resLFC_reactive <- reactive({
  ...
  resLFC <- lfcShrink(dds, coef="vital_status_Dead_vs_Alive", type="apeglm")
  return(resLFC)
})
```
...
downregulated_count <- reactive({
  ...
  downregulated_genes <- res_volcano_data %>%
    filter(log2FC < -input$lfc_threshold & pvalue < input$pval_threshold)
...
renderPlotly({
  ...
  if (!check$valid) {
    plot_ly() %>% 
      add_text(x = 0.5, y = 0.5, text = check$message, 
               textfont = list(size = 16, color = "red"),
               showlegend = FALSE) %>%
      layout(xaxis = list(visible = FALSE), 
             yaxis = list(visible = FALSE),
             title = "Volcano Plot - Insufficient Data")
  } else {
    res_volcano_data <- res_volcano_data_reactive()
    
    if (is.null(res_volcano_data)) {
      plot_ly() %>% 
        add_text(x = 0.5, y = 0.5, text = "Error generating DEG analysis", 
                 textfont = list(size = 16, color = "red"),
                 showlegend = FALSE) %>%
        layout(xaxis = list(visible = FALSE), 
               yaxis = list(visible = FALSE),
               title = "Volcano Plot - Analysis Error")
    } else {
      # Filter data based on slider inputs
      filtered_data <- res_volcano_data %>%
        mutate(
          Significance = case_when(
            log2FC > input$lfc_threshold & pvalue < input$pval_threshold ~ "Both Significant (Upregulated)",
            log2FC < -input$lfc_threshold & pvalue < input$pval_threshold ~ "Both Significant (Downregulated)",
            pvalue < input$pval_threshold ~ "Significant p-value",
            abs(log2FC) > input$lfc_threshold ~ "Significant Log2FC",
      ...
      plot_ly(
        ...
        text = ~Gene,
        color = ~Significance,
        colors = c("blue", "red", "gray", "lightgreen","lightblue"),
        type = "scatter",
        mode = "markers"
      ) %>%
        layout(
          ...
          shapes = list(
            list(type = "line", 
                 x0 = input$lfc_threshold, 
                 x1 = input$lfc_threshold, 
                 y0 = 0, 
...
renderDT({
  ...
  if (!check$valid) {
    ...
    datatable(
      data.frame(Message = check$message),
      options = list(
        pageLength = 1,
        dom = 't',
        ordering = FALSE
...
renderPlotly({
  ...
  if (!check$valid) {
    plot_ly() %>% 
      ...
      layout(xaxis = list(visible = FALSE), 
             yaxis = list(visible = FALSE),
             title = "LFC Shrinkage Volcano Plot - Insufficient Data")
  } else {
    res_LFC_volcano_data <- res_LFC_volcano_data_reactive()
    
    if (is.null(res_LFC_volcano_data)) {
      plot_ly() %>% 
        add_text(x = 0.5, y = 0.5, text = "Error generating LFC shrinkage analysis", 
                 textfont = list(size = 16, color = "red"),
                 showlegend = FALSE) %>%
        layout(xaxis = list(visible = FALSE), 
               yaxis = list(visible = FALSE),
               title = "LFC Shrinkage Volcano Plot - Analysis Error")
    } else {
      # Filter data based on slider inputs
...
### Hallmark Pathways by NES (Dead vs. Alive)
...
renderPlotly({
  ...
  if (!check$valid) {
    plot_ly() %>% 
      ...
      layout(xaxis = list(visible = FALSE), 
             yaxis = list(visible = FALSE),
             title = "Hallmark Pathways - Insufficient Data")
  } else {
    hall_data <- hall_gsea_results_reactive()
    ...
    if (is.null(hall_data)) {
      ...
      if (nrow(hall_gsea_results) == 0) {
        ...
        plot_ly(
          ...
          layout(
            title = "Hallmark Pathways by NES (Dead vs. Alive)",
...
renderPlotly({
  ...
  if (!check$valid) {
    plot_ly() %>% 
      ...
      layout(xaxis = list(visible = FALSE), 
             yaxis = list(visible = FALSE),
             title = "Enrichment Plot - Insufficient Data")
  } else {
    hall_data <- hall_gsea_results_reactive()
    ...
    if (is.null(hall_data) || is.null(input$hall_pathway)) {
      plot_ly() %>% 
        ...
        layout(xaxis = list(visible = FALSE), 
               yaxis = list(visible = FALSE),
               title = "Enrichment Plot - Analysis Error")
    } else {
...
<!-- # Reactome Pathways {data-navmenu="Menu" data-orientation="rows"} -->

<!-- ```{r reactome gmt,include=FALSE} -->
<!-- reacpathways <- gmtPathways(gmt.file = "data/c2.cp.reactome.v2023.2.Hs.symbols.gmt") -->
<!-- reacfgseaResults <- fgsea(pathways = reacpathways,stats = pvalrankData) -->
<!-- reacfgseaResults$gene_rank_positions <- sapply(reacfgseaResults$leadingEdge, function(genes) { -->
<!--   which(names(pvalrankData) %in% genes) # Get ranks for all leading-edge genes -->
<!-- }) -->
<!-- reactomefgseares<- reacfgseaResults[order(pval),] %>%  -->
<!--   select(pathway, pval, padj, ES, NES, size, gene_rank_positions) %>%  -->
<!--   arrange(desc(NES))  -->
<!-- reac_gsea_results <- reactomefgseares %>% mutate(shortpathway=gsub("REACTOME_","",reactomefgseares$pathway)) %>%  -->
<!--   na.omit() %>% -->
<!--   mutate( -->
<!--     GeneSet = reorder(shortpathway, NES), -->
<!--     Group = case_when( -->
...
<!-- ```{r select reactome pathway} -->
<!-- # Dropdown for selecting pathway -->
<!-- # Define a lookup for display names -->
<!-- display_names <- gsub("REACTOME_","",reac_gsea_results$pathway)  -->

<!-- # Create choices using the lookup -->
<!-- selectInput( -->
<!--   inputId = "reac_pathway", -->
<!--   label = "Select a Pathway:", -->
<!--   choices = setNames(reac_gsea_results$pathway, display_names), -->
<!--   selected = reac_gsea_results$pathway[1] -->
<!-- ) -->
<!-- ``` -->

<!-- ## Row {data-height="800"} -->

<!-- ### Reactome Pathways by NES (Organoid vs. Tumor) -->
...
<!--   y = ~GeneSet, -->
<!--   color = ~Group, -->
<!--   colors = c("Downregulated" = "blue", "Upregulated" = "red"), -->
<!--   type = "bar", -->
<!--   orientation = "h" -->
<!-- ) %>% -->
<!--   layout( -->
<!--     title = "Reactome Pathways by NES", -->
<!--     xaxis = list(title = "Normalized Enrichment Score (NES)"), -->
<!--     yaxis = list(title = ""), -->
<!--     barmode = "stack", -->
<!--     legend = list(title = list(text = "Group")) -->
...
<!-- ```{r reactome enrich plot} -->
<!-- renderPlotly({ -->
<!--     req(input$reac_pathway)  # Ensure input$pathway is not NULL or empty -->

<!--     # Extract the selected pathway -->
<!--     pathwayofinterest <- reacpathways[[input$reac_pathway]] -->
...
<!--     ) %>%  -->
<!--       layout( -->
<!--         title = paste("Enrichment Plot:",input$reac_pathway), -->
<!--         xaxis = list(title = "Rank"), -->
<!--         yaxis = list(title = "Enrichment Score"), -->
<!--         autosize = TRUE,  # Enable responsive resizing -->
<!--         width = NULL, -->
...
<!-- ```{r round reactome, include=FALSE} -->
<!-- reactome_rounded <- reactomefgseares %>%  -->
<!--   select(-gene_rank_positions,-size) %>% -->
<!--   as.data.frame() %>%  -->
<!--   mutate( -->
<!--   pathway = gsub("REACTOME_","",reactomefgseares$pathway) -->
<!--   ) -->

<!-- # Round all numeric columns to 2 decimal places -->
<!-- reactome_rounded[sapply(reactome_rounded, is.numeric)] <- lapply( -->
<!--   reactome_rounded[sapply(reactome_rounded, is.numeric)], -->
<!--   function(x) round(x, 2) -->
<!-- ) -->

<!-- # Keep pvalue and padj columns in scientific notation -->
<!-- reactome_rounded$pval <- formatC(reactomefgseares$pval, format = "e", digits = 2) -->
<!-- reactome_rounded$padj <- formatC(reactomefgseares$padj, format = "e", digits = 2) -->
<!-- ``` -->

<!-- ```{r reactome GSEA table} -->
<!-- datatable( -->
<!--   reactome_rounded, -->
<!--   options = list( -->
<!--     pageLength = 10,    # Number of rows per page -->
...
# KEGG Pathways {data-navmenu="Menu" data-orientation="rows"}

```{r kegg gmt reactive}
# Reactive expression for KEGG pathway analysis
kegg_gsea_results_reactive <- reactive({
  check <- sample_count_check()
  if (!check$valid) {
    return(NULL)
  }
  
  pval_data <- pvalrankData_reactive()
  if (is.null(pval_data)) {
    return(NULL)
  }
  
  tryCatch({
    keggpathways <- gmtPathways(gmt.file = "data/c2.cp.kegg_legacy.v2023.2.Hs.symbols.gmt")
    keggfgseaResults <- fgsea(pathways = keggpathways,stats = pval_data)
    keggfgseaResults$gene_rank_positions <- sapply(keggfgseaResults$leadingEdge, function(genes) {
      which(names(pval_data) %in% genes) # Get ranks for all leading-edge genes
    })
    
    keggfgseares<- keggfgseaResults[order(pval),] %>% 
      select(pathway, pval, padj, ES, NES, size, gene_rank_positions) %>% 
      arrange(desc(NES))
    
    kegg_gsea_results <- keggfgseares %>% mutate(shortpathway=gsub("KEGG_","",keggfgseares$pathway)) %>% 
      na.omit() %>%
      mutate(
        GeneSet = reorder(shortpathway, NES),
        Group = case_when(
    ...
    return(list(
      ...
      keggfgseares = keggfgseares,
      kegg_gsea_results = kegg_gsea_results,
      pvalrankData = pval_data
    ))
  }, error = function(e) {
    warning(paste("Error in KEGG pathway analysis:", e$message))
    return(NULL)
  })
})
```
...
```{r select kegg pathway}
# Dropdown for selecting pathway
renderUI({
  kegg_data <- kegg_gsea_results_reactive()
  
  if (is.null(kegg_data)) {
    p("Please select at least 3 samples for pathway analysis", style = "color: red;")
  } else {
    kegg_gsea_results <- kegg_data$kegg_gsea_results
    
    if (nrow(kegg_gsea_results) == 0) {
      p("No significant pathways found", style = "color: orange;")
    } else {
      # Define a lookup for display names
      display_names <- gsub("KEGG_","",kegg_gsea_results$pathway) 
      
      # Create choices using the lookup
      selectInput(
        inputId = "kegg_pathway",
        label = "Select a Pathway:",
        choices = setNames(kegg_gsea_results$pathway, display_names),
        selected = kegg_gsea_results$pathway[1]
      )
    }
  }
})
...
### KEGG Pathways by NES (Dead vs. Alive)
...
renderPlotly({
  ...
  if (!check$valid) {
    plot_ly() %>% 
      ...
      layout(xaxis = list(visible = FALSE), 
             yaxis = list(visible = FALSE),
             title = "KEGG Pathways - Insufficient Data")
  } else {
    kegg_data <- kegg_gsea_results_reactive()
    ...
    if (is.null(kegg_data)) {
      plot_ly() %>% 
        ...
        layout(xaxis = list(visible = FALSE), 
             yaxis = list(visible = FALSE),
             title = "KEGG Pathways - Analysis Error")
    } else {
      kegg_gsea_results <- kegg_data$kegg_gsea_results
      ...
      if (nrow(kegg_gsea_results) == 0) {
        plot_ly() %>% 
          ...
          layout(xaxis = list(visible = FALSE), 
                 yaxis = list(visible = FALSE),
                 title = "KEGG Pathways - No Significant Results")
      } else {
        plot_ly(
          kegg_gsea_results,
          x = ~NES,
...
renderPlotly({
  ...
  if (!check$valid) {
    ...
    if (is.null(kegg_data) || is.null(input$kegg_pathway)) {
      plot_ly() %>% 
        add_text(x = 0.5, y = 0.5, text = "Error generating enrichment plot", 
                 textfont = list(size = 16, color = "red"),
                 showlegend = FALSE) %>%
      ...
      keggpathways <- kegg_data$keggpathways
      pvalrankData <- kegg_data$pvalrankData
      
      # Extract the selected pathway
      pathwayofinterest <- keggpathways[[input$kegg_pathway]]
      ...
      ggplotly(
        ...
        layout(
          title = paste("Enrichment Plot:",input$kegg_pathway),
          xaxis = list(title = "Rank"),
          yaxis = list(title = "Enrichment Score"),
          autosize = TRUE,  # Enable responsive resizing
          width = NULL,
...
```{r round kegg reactive}
# Reactive expression for rounded KEGG results
kegg_rounded_reactive <- reactive({
  kegg_data <- kegg_gsea_results_reactive()
  if (is.null(kegg_data)) {
    return(NULL)
  }
  
  keggfgseares <- kegg_data$keggfgseares
  
  kegg_rounded <- keggfgseares %>% 
    select(-gene_rank_positions,-size) %>%
    as.data.frame() %>% 
    mutate(
    pathway = gsub("KEGG_","",keggfgseares$pathway)
    )
  
  # Round all numeric columns to 2 decimal places
  kegg_rounded[sapply(kegg_rounded, is.numeric)] <- lapply(
    kegg_rounded[sapply(kegg_rounded, is.numeric)],
    function(x) round(x, 2)
  )
  
  # Keep pvalue and padj columns in scientific notation
  kegg_rounded$pval <- formatC(keggfgseares$pval, format = "e", digits = 2)
  kegg_rounded$padj <- formatC(keggfgseares$padj, format = "e", digits = 2)
...
renderDT({
  ...
  if (!check$valid) {
    ...
    kegg_rounded <- kegg_rounded_reactive()
    
    if (is.null(kegg_rounded)) {
      datatable(
        data.frame(Message = "Error generating KEGG pathway analysis"),
        options = list(
          pageLength = 1,
          dom = 't',
          ordering = FALSE
